...
MODE_WEIGHTS: dict[RoutingMode, ScoreWeights] = {
    ...
    RoutingMode.CHEAP: ScoreWeights(capability=0.20, health=0.15, latency=0.10, cost=0.40, quota=0.10, priority=0.05),
    RoutingMode.QUALITY: ScoreWeights(capability=0.55, health=0.15, latency=0.05, cost=0.05, quota=0.15, priority=0.05),
    RoutingMode.QUOTA: ScoreWeights(capability=0.20, health=0.15, latency=0.05, cost=0.05, quota=0.50, priority=0.05),
}
...
def score_routing_candidate(
    *,
    ...
    return RoutingScore(total, capability, health, latency, cost, quota, priority_signal, selected_mode)
